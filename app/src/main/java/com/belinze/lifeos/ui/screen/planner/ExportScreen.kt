...
private val DATE_WINDOWS = listOf("all" to "All Time", "week" to "This Week", "month" to "This Month", "last30" to "Last 30 Days")
        ...
        Column(
            ...
            GlassCard {
                ...
                Row(
                    ...
                    DATE_WINDOWS.forEach { (key, label) ->
                        FilterChip(
                            selected = dateWindow == key,
                            onClick = { dateWindow = key },
                            label = { Text(label) },
                        )
                    }
                }
            ...
            GlassCard {
                ...
                LazyRow(horizontalArrangement = Arrangement.spacedBy(Spacing.sm)) {
                    items(PREVIEW_DOMAINS) { domain ->
                        val active = if (format == ExportFormat.CSV) domain.key == "transactions"
                                     else domain.key in selectedDomains
                        val locked = format == ExportFormat.CSV && domain.key != "transactions"
                        Column(
                            modifier = Modifier
                                .size(width = 92.dp, height = 92.dp)
                                .background(domain.color.copy(alpha = if (active) 0x30 / 255f else 0x15 / 255f), MaterialTheme.shapes.large)
                                .then(
                                    if (locked) Modifier else Modifier.clickable {
                                        selectedDomains = if (domain.key in selectedDomains)
                                            selectedDomains - domain.key else selectedDomains + domain.key
                                    }
                                )
                                .padding(Spacing.sm),
                            ...
                            Icon(Icons.Filled.Wallet, contentDescription = null,
                                tint = domain.color, modifier = Modifier.size(16.dp))
