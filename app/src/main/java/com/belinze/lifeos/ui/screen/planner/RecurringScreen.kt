...
// ─────────────────────────────────────────────────────────────────────────────
// RecurringScreen — matches RecurringScreen.tsx
// ─────────────────────────────────────────────────────────────────────────────
...
fun RecurringScreen(
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        PageScaffold(
            ...
            if (state.recurringRules.isEmpty()) {
                Box(modifier = Modifier.fillMaxWidth().padding(Spacing.x2l),
                    ...
                    Text("No recurring rules yet. Tap + to add one.",
                        ...
                        FrostCard(modifier = Modifier.fillMaxWidth().padding(vertical = Spacing.xs)) {
                            Row(
                                ...
                                Column(modifier = Modifier.weight(1f)) {
                                    ...
                                    Text(
                                        text  = "${rule.cadence ?: "monthly"} · next ${rule.nextRunAt?.take(10) ?: "—"}",
                                ...
                                Column(horizontalAlignment = Alignment.End) {
                                    Text(
                                        text  = rule.amount?.let { formatCurrency(it) } ?: "—",
                                        style = MaterialTheme.typography.titleMedium,
                                        color = MaterialTheme.colorScheme.onBackground,
                                    )
                                    Text(
                                        text  = if (rule.enabled != 0) "Active" else "Paused",
                                        fontSize = 10.sp,
                                        color = if (rule.enabled != 0) Color(0xFF10B981)
                            ...
                            Row(
                                ...
                                IconButton(onClick = { navController.navigate(NavTo.recurringForm(rule.id)) },
        ...
        ExtendedFloatingActionButton(
            onClick        = { navController.navigate(NavTo.recurringForm()) },
            text           = { Text("Add Rule") },
