...
class SmsProcessWorkerTest {
    ...
    fun setUp() {
        ...
        // Single-writer architecture: DbWriter executes on an attached connection.
        // In production this is Room's writableDatabase; tests attach an equivalent
        // framework SupportSQLiteDatabase backed by a fresh file.
            ...
            androidx.sqlite.db.SupportSQLiteOpenHelper.Configuration.builder(ctx)
                ...
                .callback(object : androidx.sqlite.db.SupportSQLiteOpenHelper.Callback(1) {
                    override fun onCreate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
                        // Production gets these from the Room schema (v3 entities);
                        // mirror them here exactly.
                        db.execSQL("""
                            CREATE TABLE IF NOT EXISTS `sms_ingest_queue` (
                                ...
                                `last_error` TEXT, `received_at` TEXT, `next_retry_at` TEXT,
                                `claimed_at` TEXT, `sender_address` TEXT NOT NULL DEFAULT ''
                            )
                        """.trimIndent())
                        ...
                        db.execSQL("CREATE INDEX IF NOT EXISTS `index_sms_ingest_queue_body_hash` ON `sms_ingest_queue` (`body_hash`)")
                        db.execSQL("""
                            CREATE TABLE IF NOT EXISTS `import_audit` (
                                `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                                `mpesa_code` TEXT, `raw_message` TEXT, `amount` REAL,
                                `merchant` TEXT, `outcome` TEXT, `failure_reason` TEXT,
                    ...
                    override fun onUpgrade(
                        db: androidx.sqlite.db.SupportSQLiteDatabase,
                            oldVersion: Int,
                            newVersion: Int,
                        ) { /* legacy DBs never upgrade in tests */ }
                })
                .build()
        )
        DbWriter.attachForTest(helper.writableDatabase)
        ...
        // merchant_categories is a Room-owned table (normally created by the app's
        // Room schema). DbWriter.lookupMerchantCategory reads it during inserts.
        db.execForTest(
            """
    ...
    fun `non-mpesa SMS is ignored and does not reach transactions`() = runBlocking {
        ...
        val audit = db.getAuditLog(10)
        assertTrue(
            audit.any { it["outcome"] == "ignored_not_financial" },
            "audit must record ignored_not_financial"
        )
    }
    ...
    fun `quarantined SMS is held in audit and not inserted to ledger`() = runBlocking {
        ...
        // Production broadcasts always carry the real sender ID; the body alone
        // has no institution keyword, so provide it as the receiver would.
        val queueId = db.enqueueIngest(sms, sender = "MPESA")
        val result = buildWorker(queueId).doWork()
