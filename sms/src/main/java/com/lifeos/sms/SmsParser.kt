...
object SmsParser {
    ...
    data class ParsedTransaction(
        ...
        /**
         * True when the transaction code was synthesised (no real code in the SMS).
         * Synthesised codes are stable per-body but carry lower confidence and are
         * never matched against dedup indexes that hold real codes.
         */
        val isSynthesisedCode: Boolean = false,
        /**
         * True when a SENT/AIRTIME/PAYBILL/BUY_GOODS/WITHDRAW transaction was funded via
         * Fuliza M-PESA overdraft. The SMS body will contain the word "fuliza".
         * Used by the ingestion pipeline to record a corresponding Fuliza draw.
         */
        val isFulizaFunded: Boolean = false,
        /**
         * For FULIZA_CHARGE: the amount drawn from Fuliza that triggered this charge notice.
         * Distinct from [amount] which holds the access fee charged in this period.
         */
        val fulizaAmountUsedKes: Double? = null,
        /**
         * For FULIZA_CHARGE: deadline by which Fuliza must be repaid (epoch millis).
         * Parsed from "Repay by DD/MM/YY" in the charge-notice SMS body.
         */
        val fulizaDueDateMs: Long? = null,
    ) {
    ...
    private val OFFICIAL_MPESA_SENDERS = setOf("MPESA", "M-PESA", "M_PESA")

    // Categories where a "fuliza" mention in the body means the transaction was funded by Fuliza.
    private val FULIZA_DRAW_CATEGORIES = setOf(
        SmsParserConfig.SmsCategory.SENT,
        SmsParserConfig.SmsCategory.AIRTIME,
        SmsParserConfig.SmsCategory.PAYBILL,
        SmsParserConfig.SmsCategory.BUY_GOODS,
    ...
    // Used by pickTransactionAmount to identify the transaction-amount verb proximity.
    private val ACTION_VERB_RE = Regex(
        """(?:sent\s+to|received\s+from|paid\s+to|bought\s+from?|withdraw[an]l?\s+at|deposited?\s+(?:to|from)|borrowed\s+from)""",
    ...
    private val COUNTERPARTY_VIA_KOPO        =
        Regex("""\s+via\s+(?:kopo[\s-]+kopo|kopokopo)(?:\s+ltd|\s+limited)?.*$""", RegexOption.IGNORE_CASE)
    ...
    private val COUNTERPARTY_TRAILING_ACCOUNT =
        Regex("""\s+(?:for\s+)?(?:account|acc\.?|acct\.?|account\s+number|meter|ref\.|reference|policy|token|bill)\s*[:#]?\s*[\w-]+.*$""", RegexOption.IGNORE_CASE)
    ...
    private val KEYWORD_HINTS: Array<Pair<String, String>> = arrayOf(
        ...
        // Account-number markers are a stronger paybill signal than "paid to" —
        // must be hinted BEFORE buy_goods or Kopokopo-style paybills misclassify.
        "for account" to "paybill",
        "account number" to "paybill",
    ...
    private fun detectRule(body: String, bodyLower: String): MatchResult? {        // Fast path: keyword hint → try hinted rule's primary patterns first
        ...
        for (rule in SmsParserConfig.activeRules()) {
            if (rule.patterns.any { it.containsMatchIn(body) }) {
                return MatchResult(rule, SmsParserConfig.Confidence.HIGH, phase = 1)
            }
        }
        ...
        return ruleId?.let { id ->
            SmsParserConfig.activeRules().find { it.id == id }
    ...
    internal fun scoreConfidence(
        ...
        rulePhase: Int,
        balanceAfter: Double?,
        isSynthesisedCode: Boolean = false,
        ...
        // Synthesised codes (no real code in SMS) carry lower score → routes to REVIEW.
        val codeScore = when {
            isSynthesisedCode -> 0.55
            CODE_FORMAT_RE.matches(code) -> 1.0
            else -> 0.5
        }
        ...
        // WITHDRAW floor: ATM and M-Pesa agent SMSes rarely include a balance line,
        // so the −0.20 high-value penalty can push an otherwise-valid withdrawal
        // into QUARANTINE. Withdrawals are unambiguously categorised once the rule
        // matches, so cap at MEDIUM — never quarantine them.
        if (category == SmsParserConfig.SmsCategory.WITHDRAW && score < 0.65) {
            return SmsParserConfig.Confidence.MEDIUM
        }
    ...
    /**
     * Selects the transaction amount when multiple Ksh/KES figures appear.
     *
     * Strategy:
     *  1. If only one AMOUNT_RE match → return it (no ambiguity).
     *  2. Filter out amounts that overlap a BALANCE_RE or FEE_RE match (those are
     *     balance/fee figures, not the transaction amount).
     *  3. If multiple candidates remain, pick the one whose character position is
     *     closest to an action verb (sent to / received from / paid to / …).
     *  4. Fallback: first surviving candidate, then all-candidates if filtering
     *     eliminated everything.
     */
    private fun pickTransactionAmount(body: String): Double? {
        val allMatches = SmsParserConfig.AMOUNT_RE.findAll(body).toList()
        if (allMatches.isEmpty()) return null
        if (allMatches.size == 1) {
            return allMatches[0].groupValues[1].replace(",", "").toDoubleOrNull()?.takeIf { it > 0 }
        }

        // Ranges covered by balance or fee patterns — amounts overlapping these are excluded.
        val excludedRanges = (SmsParserConfig.BALANCE_RE.findAll(body) + SmsParserConfig.FEE_RE.findAll(body))
            .map { it.range }
            .toList()

        val candidates = allMatches.filter { amtMatch ->
            excludedRanges.none { excl ->
                amtMatch.range.first <= excl.last && amtMatch.range.last >= excl.first
            }
        }.ifEmpty { allMatches }

        if (candidates.size == 1) {
            return candidates[0].groupValues[1].replace(",", "").toDoubleOrNull()?.takeIf { it > 0 }
        }

        val verbPositions = ACTION_VERB_RE.findAll(body).map { it.range.first }.toList()
        val best = if (verbPositions.isEmpty()) {
            candidates[0]
        } else {
            candidates.minByOrNull { m -> verbPositions.minOf { vp -> Math.abs(m.range.first - vp) } }
                ?: candidates[0]
        }
        return best.groupValues[1].replace(",", "").toDoubleOrNull()?.takeIf { it > 0 }
    }
    ...
    private fun parseAmountForCategory(body: String, category: SmsParserConfig.SmsCategory): Double? {
        return when (category) {
            SmsParserConfig.SmsCategory.FULIZA_CHARGE -> {
                // Prefer the access/maintenance fee; fall back to verb-proximity selection.
                SmsParserConfig.FULIZA_ACCESS_FEE_RE.find(body)
                    ?.groupValues?.get(1)?.replace(",", "")?.toDoubleOrNull()
                    ?: pickTransactionAmount(body)
            }
            ...
            else -> pickTransactionAmount(body)
        }
    }
    ...
    internal fun parseDate(body: String, receivedAtMs: Long): Long {
        return try {
            ...
            // Clamp any parsed date that is more than 24 h in the future relative to the
            // SMS received timestamp back to the timestamp.  Reference numbers that look
            // like dates (e.g. "15/07/26") would otherwise land transactions in 2026.
            val futureThresholdMs = receivedAtMs + 24 * 60 * 60 * 1_000L
    ...
    fun parse(
        ...
        return try {
            // Normalize before any regex: strips zero-width chars, repairs concatenation
            // artefacts ("Confirmed.on", "PMWithdraw"), removes promo tails.
            val body = SmsNormalizer.normalize(sms)
            ...
            // Stage 0f: Failed transaction filter — prevents phantom ledger entries.
            // "Failed. You do not have enough money…" has a code + amount and would
            // otherwise be synthesised into a real transaction row.
            if (ServiceNoticeFilter.isFailedTransaction(body)) {
                return SmsParseResult.Error(
                    SmsParseError("failed_transaction", sms).also { RejectionLog.record(it) }
                )
            }

            // Stage 1: M-Pesa code required.
            // Primary:   mixed-alphanumeric codes (must have ≥1 letter AND ≥1 digit).
            // Fallback1: all-alpha codes (e.g. UGFDLBONAZ) — no digit, always precede "Confirmed".
            // Fallback2: synthesise a stable code when sender is the official MPESA shortcode.
            //            Synthesised codes carry codeScore=0.55 → routes to REVIEW, not DIRECT.
            val rawCode = SmsParserConfig.CODE_RE.find(body)?.groupValues?.get(1)
                ?: SmsParserConfig.CODE_START_RE.find(body)?.groupValues?.get(1)
            val isSynthesisedCode: Boolean
            val code: String
            if (rawCode != null) {
                code = rawCode
                isSynthesisedCode = false
            } else {
                val senderUp = sender.uppercase()
                val isOfficialSender = OFFICIAL_MPESA_SENDERS.any { senderUp.contains(it) }
                if (isOfficialSender) {
                    // Stable 9-char synthetic code: "F" + first 8 hex chars of SHA-256(body).
                    code = "F" + sha256(body).take(8).uppercase()
                    isSynthesisedCode = true
                } else {
                    return SmsParseResult.Error(
            ...
            val match = detectRule(body, bodyLower) ?: run {
                ...
                val confidence = scoreConfidence(
                    SmsParserConfig.SmsCategory.UNKNOWN, amount, date, null, code, 0, balance, isSynthesisedCode
                )
                ...
                return SmsParseResult.Success(
                    ParsedTransaction(
                        ...
                        isSynthesisedCode = isSynthesisedCode,
                    )
                )
            }
            ...
            // Amount drawn from Fuliza that generated this charge (distinct from the fee).
            val fulizaAmountUsed: Double? = when (match.rule.category) {
                SmsParserConfig.SmsCategory.FULIZA_CHARGE ->
                    SmsParserConfig.FULIZA_AMOUNT_USED_RE.find(body)
                        ?.groupValues?.get(1)?.replace(",", "")?.toDoubleOrNull()
                else -> null
            }
            // Repayment deadline stated in the charge-notice SMS.
            val fulizaDueDate: Long? = when (match.rule.category) {
                SmsParserConfig.SmsCategory.FULIZA_CHARGE ->
                    SmsParserConfig.FULIZA_DUE_DATE_RE.find(body)
                        ?.groupValues?.get(1)?.let { parseDateMatch(it, null) }
                else -> null
            }
            ...
            val confidence = scoreConfidence(
                match.rule.category, finalAmount, date, counterparty, code, match.phase, balance, isSynthesisedCode
            )
            ...
            // Stage 5g: Fuliza-funded draw detection — SENT/AIRTIME/PAYBILL/BUY_GOODS/WITHDRAW
            // funded via Fuliza overdraft have "fuliza" in the body.
            val isFulizaFunded = match.rule.category in FULIZA_DRAW_CATEGORIES &&
            ...
            SmsParseResult.Success(
                ParsedTransaction(
                    ...
                    isSynthesisedCode       = isSynthesisedCode,
                    isFulizaFunded          = isFulizaFunded,
                    fulizaAmountUsedKes     = fulizaAmountUsed,
                    fulizaDueDateMs         = fulizaDueDate,
                )
            )
        } catch (e: Exception) {
