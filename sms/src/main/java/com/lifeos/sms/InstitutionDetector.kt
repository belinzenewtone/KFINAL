...
object InstitutionDetector {
    ...
    private val INSTITUTIONS = listOf(
        ...
        Institution(
            // I&M Bank Kenya
            id           = "im",
            senderIds    = setOf("IMBANK", "IMBANKKE", "I&MBANK", "IANDMBANK", "IMBANK-NEWS"),
    ...
    fun detect(sender: String, body: String): Detection? {
        ...
        // The character after the matched ID must be alphanumeric or '-' so
        // underscore-suffixed promo senders (e.g. AIRTEL_HOME) are NOT treated
        // as financial institutions.
        for (inst in INSTITUTIONS) {
            if (inst.senderIds.any { id ->
                    val idx = upper.indexOf(id)
                    idx >= 0 && run {
                        val next = idx + id.length
                        next == upper.length ||
                            upper[next].isLetterOrDigit() ||
                            upper[next] == '-'
                    }
                }) {
