...
internal object GenericBankParser : FinancialSmsParser {
    ...
    private val DEBIT_RE = Regex(
        """(?i)\b(?:sent|paid|payment|debited?|deducted?|purchas\w+|bought|send\s+to|outward\s+(?:transfer|remittance)|funds?\s+(?:sent|transferred))\b"""
    )
    private val TRANSFER_RE = Regex(
        """(?i)\b(?:transferr?ed?|transfer\s+to|transfer\s+from|funds?\s+transfer)\b"""
    )
    ...
    // The optional mask group handles "account 100****882 has been credited".
    private val ACCOUNT_CREDITED_RE = Regex("""(?i)\baccount\s+(?:[0-9X*]+\s+)?(?:has been\s+)?credited\b""")
    private val ACCOUNT_DEBITED_RE  = Regex("""(?i)\baccount\s+(?:[0-9X*]+\s+)?(?:has been\s+)?debited\b""")
    ...
    private val FEE_RE = Regex(
        """(?i)(?:charges?(?:\s+charged\s+is)?|fees?|commission|levy|excise)[:\s]+(?:k\.?shs?\.?|kes\.?)?\s*([0-9,]+(?:\.[0-9]{1,2})?)"""
    )

    // ── Card debit detection (Absa, StanChart, Equity card purchases) ────
    // Without this every card-purchase SMS is UNKNOWN → QUARANTINE.
    private val CARD_DEBIT_RE = Regex(
        """(?i)(?:transaction\s+of\s+(?:k\.?shs?\.?|kes\.?)\s*[0-9,]+\s+was\s+made\s+on\s+(?:your\s+)?card|""" +
        """approved\s+on\s+your\s+card\s+ending|""" +
        """card\s+(?:purchase|payment|transaction)\s+of|""" +
        """pos\s+(?:purchase|debit)\s+(?:k\.?shs?\.?|kes\.?))"""
    )
    ...
    fun parseWithDetection(
        ...
        val isCardDebit = CARD_DEBIT_RE.containsMatchIn(body)
        val isCredit   = isCrDr ||
            CREDIT_RE.containsMatchIn(body) ||
            ACCOUNT_CREDITED_RE.containsMatchIn(body)
        val isDebit    = isCardDebit ||
            isDrDr ||
        ...
        val category = when {
            ...
            // Bank SMS that says "credited" AND embeds an M-Pesa ref is the receive-side
            // confirmation of an M-Pesa-to-bank transfer — always RECEIVED.
            isCredit && isDebit && crossRefMpesaCode != null -> SmsParserConfig.SmsCategory.RECEIVED
        ...
        // Use the parsed transaction date (not receivedAtMs) so re-delivery of the same
        // SMS at a different time does not produce a different hash.
        // Format matches M-Pesa rows: "sem_" + first 16 hex of SHA-256.
        val txDateMs    = SmsParser.parseDate(body, receivedAtMs)
        val semanticHash = "sem_" + SmsParser.sha256(
            "$instId|${category.name}|${amount.toLong()}|${txDateMs / 60_000}|${counterparty.orEmpty()}"
        ).take(16)
        ...
        return SmsParser.SmsParseResult.Success(
            SmsParser.ParsedTransaction(
                ...
                fee          = fee,
                date         = SmsParser.parseDate(body, receivedAtMs),
                rawSms       = body,
                parseRoute   = route,
