...
internal object AirtelMoneyParser : FinancialSmsParser {
    ...
    override fun parse(body: String, sender: String, receivedAtMs: Long): SmsParser.SmsParseResult {
        ...
        // Use parsed tx date (not receivedAtMs) so re-delivery at a different time
        // doesn't defeat dedup.  Format matches M-Pesa/bank rows: sem_ + 16 hex chars.
        val txDateMs    = SmsParser.parseDate(body, receivedAtMs)
        val semanticHash = "sem_" + SmsParser.sha256(
            "airtel|${category.name}|${amount.toLong()}|${txDateMs / 60_000}|${counterparty.orEmpty()}"
        ).take(16)
