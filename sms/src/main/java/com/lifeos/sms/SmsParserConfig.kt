...
object SmsParserConfig {
    ...
    /**
     * M-Pesa transaction confirmation code — 9-10 alphanumeric chars (mixed case
     * accepted) that must contain AT LEAST ONE LETTER AND AT LEAST ONE DIGIT.
     * The double lookahead eliminates false matches on:
     *  - 10-digit phone numbers (all digits, no letter)
     *  - numeric agent codes and account references (all digits)
     *  - English words like "Confirmed" (no digit)
     * Mixed-case codes (e.g. "Qhl6zkmf10") are accepted — Safaricom occasionally
     * issues lowercase-prefixed codes.
     */
    val CODE_RE = Regex("""\b(?=[A-Za-z0-9]*[A-Za-z])(?=[A-Za-z0-9]*[0-9])([A-Za-z0-9]{9,10})\b""")

    /**
     * Fallback for all-alpha transaction codes (e.g. UGFDLBONAZ) that carry no digit
     * and cannot satisfy CODE_RE's digit lookahead. These always appear at the very
     * start of the SMS body immediately before the word "Confirmed".
     */
    val CODE_START_RE = Regex("""^([A-Za-z]{9,10})\s+(?=[Cc]onfirmed)""")
     ...
     * "interest" is anchored to "fuliza" or "m-pesa" context to prevent matching
     * bank loan interest lines (e.g. "Interest charged Ksh 450 on your KCB loan").
     * The bare `interest charged` alternative is safe here: this regex is only
     * applied when the message has already been classified FULIZA_CHARGE.
     */
    val FULIZA_ACCESS_FEE_RE = Regex(
        """(?:access\s+fee|maintenance\s+fee|(?:fuliza|m-pesa)\s+interest|interest\s+charged)\s*(?:charged\s*)?(?:(?:Ksh|KES)\s?)?([\d,]+(?:\.\d{1,2})?)""",
        RegexOption.IGNORE_CASE,
    )
    ...
    /**
     * Amount drawn from Fuliza that triggered this charge notice.
     * Matches: "charged on Ksh X borrowed", "for borrowing Ksh X", "for using Ksh X".
     */
    val FULIZA_AMOUNT_USED_RE = Regex(
        """(?:charged\s+on|for\s+borrowing|for\s+using|amount\s+used)[:\s]+(?:Ksh|KES)\s?([\d,]+(?:\.\d{1,2})?)""",
        RegexOption.IGNORE_CASE,
    )

    /**
     * Fuliza repayment deadline in a charge-notice SMS.
     * Matches: "Repay by 15/01/25", "due by 1/1/25", "pay before 15/01/25".
     */
    val FULIZA_DUE_DATE_RE = Regex(
        """(?:repay\s+by|due\s+by|pay\s+before|repayment\s+due\s+on)\s+(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})""",
        RegexOption.IGNORE_CASE,
    )
    ...
    val FULIZA_NOTICE_RE = Regex(
        ...
        """processing fee charged|your fuliza m-pesa balance|m-pesa statement""",
        RegexOption.IGNORE_CASE,
    )
    ...
    private val SERVICE_NOTICE_RE = Regex(
        """(?:one[\s-]?time\s+password|your\s+otp\b|otp\s+for\b|verification\s+code|security\s+code|""" +
        ...
        // ── Extended patterns ported from KOTLIN GenericBankParser (v2) ──
        // Card-declined — prevents a declined purchase from appearing as real spending
        """(?:online\s+)?transaction\s+(?:of\s+(?:k\.?shs?\.?|kes\.?)\s*[0-9,]+\s+)?(?:has\s+been\s+)?declined|""" +
        """your\s+card\s+(?:has\s+been\s+)?declined|""" +
        """card\s+(?:transaction\s+)?declined|""" +
        // Loop / NCBALoop service notices
        """hey\s+looper|""" +
        """service\s+interruption\s+on\s+all\s+loop|""" +
        // StanChart maintenance
        """standard\s+chartered.{0,40}maintenance|""" +
        // Rate-change notices (not transactions)
        """base\s+lending\s+rate\s+shall\s+be\s+revised|""" +
        """interest\s+rate\s+(?:has\s+been\s+)?(?:revised|adjusted|changed)|""" +
        // Failed airtime refund confirmation (no money movement)
        """airtime\s+(?:purchase\s+)?(?:has\s+)?failed.{0,60}refund|""" +
        // Service request / ticket acknowledgement
        """(?:service\s+request|ticket|case)\s+(?:number|no\.?|#)\s*[A-Z0-9\-]{4,}|""" +
        // "Use code XXXX" — PIN reset / activation codes
        """use\s+code\s+\d{4,8}\b|""" +
        // Airtime-load-failed confirmation
        """we\s+could\s+not\s+load\s+airtime)""",
        RegexOption.IGNORE_CASE,
    )
    ...
    // ─── OTA rule-bundle hot-swap ──────────────────────────────────────
    // When a remote bundle is loaded (see BundleCompiler), activeRules()
    // serves it instead of the compiled-in DETECTION_RULES. clearBundle()
    // reverts to the static rules. Volatile: parsing runs on worker threads.
    ...
    fun activeRules(): List<DetectionRule> = activeBundle ?: DETECTION_RULES
    ...
    val DETECTION_RULES: List<DetectionRule> = listOf(
        ...
        DetectionRule(
            ...
            patterns = listOf(
                // Amount may carry a doubled currency token ("Ksh Ksh4,870.00")
                Regex("""(?:you have\s+)?received\s+(?:Ksh|KES)\s?(?:(?:Ksh|KES)\s?)?[\d,.]+\s+from\s+""", RegexOption.IGNORE_CASE),
                Regex("""(?:Ksh|KES)\s?[\d,.]+\s+received from\s+""", RegexOption.IGNORE_CASE),
                // Old / compact formats sometimes drop "You have"
                Regex("""received\s+(?:Ksh|KES)\s?(?:(?:Ksh|KES)\s?)?[\d,.]+\s+from\s+[A-Z]""", RegexOption.IGNORE_CASE),
            ...
            counterpartyPatterns = listOf(
                Regex("""received\s+(?:Ksh|KES)\s?(?:(?:Ksh|KES)\s?)?[\d,.]+\s+from\s+(.+?)(?:\s+on\s|\s+New\s|\.|$)""", RegexOption.IGNORE_CASE),
                Regex("""(?:Ksh|KES)\s?[\d,.]+\s+received from\s+(.+?)(?:\s+on\s|\s+New\s|\.|$)""", RegexOption.IGNORE_CASE),
        ...
        DetectionRule(
            ...
            counterpartyPatterns = listOf(
                // M-Shwari deposit: extract "M-Shwari Savings Account" as counterparty.
                Regex("""(?:to\s+)?(M-Shwari(?:\s+Lock)?(?:\s+Savings\s+Account)?)""", RegexOption.IGNORE_CASE),
        ...
        // 3b. M-SHWARI — BEFORE airtime/paybill: M-Shwari savings deposits and lock-savings
        //     would otherwise fall through to PAYBILL or SENT.
        DetectionRule(
            id = "mshwari_savings",
            category = SmsCategory.DEPOSIT,
            description = "M-Shwari savings deposit or lock savings",
            patterns = listOf(
                Regex("""(?:Ksh|KES)\s?[\d,.]+\s+(?:deposited|transferred|saved)\s+to\s+M-Shwari""", RegexOption.IGNORE_CASE),
                Regex("""M-Shwari.*(?:Ksh|KES)\s?[\d,.]+.*(?:saved|deposit|locked)""", RegexOption.IGNORE_CASE),
                Regex("""M-Shwari\s+(?:lock\s+)?savings\s+account""", RegexOption.IGNORE_CASE),
            ),
            fallbackPatterns = listOf(
                Regex("""M-Shwari.*savings""", RegexOption.IGNORE_CASE),
                Regex("""savings.*M-Shwari""", RegexOption.IGNORE_CASE),
                Regex("""M-Shwari\s+lock""", RegexOption.IGNORE_CASE),
            ),
            counterpartyPatterns = listOf(
                Regex("""(M-Shwari(?:\s+Lock)?(?:\s+Savings)?)""", RegexOption.IGNORE_CASE),
            ),
        ),
        ...
        // 9b. LIPA MDOGO MDOGO — Safaricom device-financing installment payments.
        //     Uses LOAN category; body always contains "Lipa Mdogo Mdogo" or "mdogo mdogo".
        DetectionRule(
            id = "lipa_mdogo_mdogo",
            category = SmsCategory.LOAN,
            description = "Lipa Mdogo Mdogo device-financing installment",
            patterns = listOf(
                Regex("""Lipa\s+Mdogo\s+Mdogo.*(?:Ksh|KES)\s?[\d,.]+.*(?:installment|payment|paid)""", RegexOption.IGNORE_CASE),
                Regex("""(?:Ksh|KES)\s?[\d,.]+.*(?:installment|payment).*Lipa\s+Mdogo\s+Mdogo""", RegexOption.IGNORE_CASE),
            ),
            fallbackPatterns = listOf(
                Regex("""Lipa\s+Mdogo\s+Mdogo""", RegexOption.IGNORE_CASE),
                Regex("""mdogo\s+mdogo""", RegexOption.IGNORE_CASE),
            ),
            counterpartyPatterns = listOf(
                Regex("""(Lipa Mdogo Mdogo)""", RegexOption.IGNORE_CASE),
            ),
        ),
        // 9c. GLOBAL PAY — M-Pesa cross-border / international transfers.
        //     Body always contains "GlobalPay", "Global Pay", or "international".
        DetectionRule(
            id = "global_pay",
            category = SmsCategory.SENT,
            description = "M-Pesa GlobalPay international transfer",
            patterns = listOf(
                Regex("""(?:Ksh|KES)\s?[\d,.]+\s+sent.*(?:GlobalPay|Global\s+Pay|international(?:ly)?)""", RegexOption.IGNORE_CASE),
                Regex("""(?:GlobalPay|Global\s+Pay).*(?:Ksh|KES)\s?[\d,.]+\s+sent""", RegexOption.IGNORE_CASE),
                Regex("""sent.*internationally.*(?:Ksh|KES)\s?[\d,.]+""", RegexOption.IGNORE_CASE),
            ),
            fallbackPatterns = listOf(
                Regex("""GlobalPay""", RegexOption.IGNORE_CASE),
                Regex("""Global\s+Pay""", RegexOption.IGNORE_CASE),
                Regex("""sent\s+internationally""", RegexOption.IGNORE_CASE),
            ),
            counterpartyPatterns = listOf(
                Regex("""(?:Ksh|KES)\s?[\d,.]+\s+sent\s+to\s+(.+?)\s+(?:in|via|on\s+\d)""", RegexOption.IGNORE_CASE),
                Regex("""(?:GlobalPay|Global\s+Pay):\s*(.+?)\s+(?:received|on\s+\d)""", RegexOption.IGNORE_CASE),
            ),
        ),
